...
Deploy via CDK (`infra/cdk/lib/backend-foundation-stack.ts`). Set `ANTHROPIC_API_KEY` on the backend Lambda in AWS (not in CloudFormation). Optional: `ANTHROPIC_FOOD_VISION_MODEL` (defaults to a Sonnet vision model). Enable the feature with `FF_PHOTO_FOOD_LOG=true` at deploy time for the stack and matching web env.
